...
export const getPosts = async (req, res) => {
  ...
  const allPosts = await Dogs.find({})
    .sort({ _id: -1 })
    .skip(startIndex)
    .limit(LIMIT);
  res.status(200).json({
...
export const getPostsBySearch = async (req, res) => {
  ...
  const searchedPosts = await Dogs.find({
    $or: [{ name }, { breed }, { tags: { $in: tags.split(",") } }],
  });
...
export const updatePosts = async (req, res) => {
  ...
  if (postExists) {
    ...
    return res
      .status(400)
      .json({ ERROR: "Post with that ID does not exists!" });
  }
};
...
export const likePosts = async (req, res) => {
  ...
  if (!req.userId)
    return res.status(409).json({ ERROR: "Unauthentiated User!" });
  ...
  if (currPost) {
    ...
    const updatedPost = await Dogs.findByIdAndUpdate(id, currPost, {
      new: true,
    });
